...
const RELEVANCE_THRESHOLD = parseFloat(process.env.RELEVANCE_THRESHOLD || "0.1");
...
export async function retrieveAcrossAllFathers(question, fatherIds) {
  ...
  let relevant = rerankedResults
    .filter(({ topScore }) => topScore >= RELEVANCE_THRESHOLD)
    .sort((a, b) => b.topScore - a.topScore);

  // Fallback: if nothing clears the threshold, return top 3 anyway
